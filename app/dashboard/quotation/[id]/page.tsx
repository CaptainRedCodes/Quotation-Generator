    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center gap-4 mb-6">
          <Link href="/dashboard" className="p-2 hover:bg-gray-100 rounded"><ArrowLeft className="w-5 h-5" /></Link>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <button onClick={() => setShowComponents(!showComponents)} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 mb-4">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {quotation.items.map((item, i) => (
                      <tr key={i} className={`border-b ${item.isProductHeader ? 'bg-gray-50' : ''}`}>
                        <td className="py-2 font-medium">{item.componentName}</td>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span>Date:</span><span>{formatDate(quotation.date)}</span></div>
                <div className="flex justify-between"><span>Subtotal:</span><span>₹{formatIndianCurrency(quotation.subtotal)}</span></div>
                ...
                <div className="flex justify-between"><span>GST ({gstPercent}%):</span><span>₹{formatIndianCurrency(quotation.gstAmount)}</span></div>
                <div className="flex justify-between font-bold border-t pt-2"><span>Total:</span><span>₹{formatIndianCurrency(quotation.totalAmount)}</span></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-2">
                {quotation.invoiceId ? (
                  <Link href={`/dashboard/invoice/${quotation.invoiceId}`} className="flex items-center justify-center gap-2 w-full px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">
